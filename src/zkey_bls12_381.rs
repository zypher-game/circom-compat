...
pub fn read_zkey<R: Read + Seek>(
    reader: &mut R,
) -> IoResult<(ProvingKey<Bls12_381>, ConstraintMatrices<Fr>)> {
...
impl<'a, R: Read + Seek> BinFile<'a, R> {
    ...
    fn proving_key(&mut self) -> IoResult<ProvingKey<Bls12_381>> {
...
impl HeaderGroth {
    ...
    fn read<R: Read>(mut reader: &mut R) -> IoResult<Self> {
        ...
        // group order r of Bls12_381
        let q = BigInteger384::deserialize_uncompressed(&mut reader)?;
